...
\begin{table}[H]
  ...
  \begin{tabular}{lrrrr}
    \toprule
    Run & Pic \(\text{metric}\) & Step & Pic \(\text{metric\_robust}\) & \(\text{exploit\_sp}\) \\
    \midrule
...
\begin{figure}[H]
  ...
  \caption{Evolució de la \(\text{metric}\) (esquerra) i la \(\text{metric\_robust}\)
           ...
           el punt de Nash mínim (1M passos de F6). F6 assoleix el pic de \(\text{metric}\)
           molt aviat (91{,}0\% @14M) gràcies al warm-start, però oscil·la
           posteriorment; la \(\text{metric\_robust}\) és comparable a F5.}
...
\begin{table}[H]
  ...
  \begin{tabular}{lrrr}
    ...
    \midrule
    \(\text{metric}\) màx   & 89{,}0\% & 89{,}2\% & \textbf{91{,}0\%} \\
    ...
    \bottomrule
  \end{tabular}
  \caption{Comparativa F4/F5/F6 (runs de 48M passos). F6 obté la millor \(\text{metric}\)
...
\(\text{metric}\) als 14M passos, +1{,}8~pp sobre F5. El warm-start des del model robust
...
\begin{itemize}
  \item \textbf{\texttt{best.zip}} (\(\text{metric}\) màxima): el model que millor
...
\begin{table}[H]
  ...
  \begin{tabular}{lcccc}
    \toprule
    Model & \texttt{wr\_pool\_mean} & \(\text{metric\_robust}\) & \(\sigma_{\text{pool}}\) & \texttt{exploit\_vs\_sl} \\
    \midrule
...
\(\text{metric\_robust}\). Els dos candidats F6 queden descartats (11{,}4 i 10{,}6~pp).
Ambdós candidats del re-run passen el criteri Nash (1{,}8~pp cadascun); el desempat per
\(\text{metric\_robust}\) selecciona \textbf{\texttt{best\_nash\_rerun}}
