...
selecció definitiva del model a desplegar es resol al punt de control~3
(secció~\ref{sec:checkpoint3}), un cop es disposi del re-run amb el criteri corregit.

\subsection*{Punt de control~3: candidats a desplegament}\label{sec:checkpoint3}

La Fase~6 produeix tres pesos candidats que representen perfils d'optimització distints.
Per comparar-los s'ha fet una avaluació independent i precisa de cadascun: 200~partides
per enfrontament i balancejades per posició (meitat mà, meitat post), molt més fiable que
les avaluacions periòdiques de 15~partides del \texttt{training\_log.csv}. Els resultats
(taula~\ref{tab:checkpoint3}) confirmen la limitació del \texttt{best\_nash} original.
...
\begin{table}[H]
  ...
  \begin{tabular}{lccc}
    ...
    \midrule
    \texttt{best.zip}~F6       & 70{,}7\% & 69{,}2\% & 12{,}0~pp \\
    \texttt{best\_nash.zip}~F6 & 69{,}8\% & 67{,}5\% & 10{,}0~pp \\
    \texttt{sl\_final.pt}~F6   & 34{,}7\% & 32{,}2\% & — \\
    \bottomrule
  \end{tabular}
  \caption{Candidats a desplegament al tancament de la Fase~6, avaluats amb 200~partides
           balancejades per posició contra les 6~variants del pool. L'\texttt{exploit\_vs\_sl}
           es mesura contra \texttt{sl\_final.pt}; per això no aplica a la fila del propi
           \ac{SL}. Cap candidat baixa del llindar de 8~pp amb avaluació fiable.}%
  \label{tab:checkpoint3}
\end{table}

\emph{Limitació detectada.} El \emph{checkpoint} \texttt{best\_nash.zip} va ser
seleccionat al pas~1M perquè en aquell moment \(\text{exploit\_vs\_sl}=3{,}3\)~pp era
el mínim de l'entrenament. Aquest valor, però, és espuri per dos motius. Primer, la
...
La taula~\ref{tab:checkpoint3} ho confirma amb una avaluació precisa: amb 200~partides
balancejades, l'\texttt{exploit\_vs\_sl} real del \texttt{best\_nash.zip} és de 10~pp,
gairebé igual que el del \texttt{best.zip} (12~pp). El \texttt{best\_nash} original, per
tant, no és menys explotable que la millor resposta. A més, el valor estructural de ~10~pp
prové en part de la feblesa del propi \ac{SL} (\texttt{sl\_final.pt} bat el pool només al
34{,}7\%): qualsevol \ac{PPO} decent hi guanya ~60~\%, cosa que limita l'\texttt{exploit\_vs\_sl}
com a indicador de Nash mentre la política mitjana no convergeixi.

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{figures/checkpoint3/comparacio_candidats.pdf}
  \caption{Win-rate dels tres candidats contra les 6~variants del pool (200~partides
           balancejades per variant). \texttt{best.zip} i \texttt{best\_nash.zip} rendeixen
           gairebé igual; \texttt{sl\_final.pt} (política mitjana \ac{SL}) queda clarament
           per sota del 50~\% a totes les variants.}%
  \label{fig:checkpoint3-candidats}
\end{figure}

\emph{Re-run planificat.} Per obtenir un \texttt{best\_nash} fiable cal introduir
tres canvis al guió d'entrenament
(\texttt{RL/entrenament/entrenamentsComparatius/fase6/entrenament\_fase6.py}):
(1)~un llindar mínim de \(10^7\) passos que impedeixi arxivar el \texttt{best\_nash}
abans del pas~10M, quan la \texttt{sl\_loss} ja s'ha estabilitzat;
(2)~pujar les partides d'avaluació de 15 a 60 i alternar-ne la posició (resolució
$\approx$1{,}7~pp i sense biaix de mà); i (3)~arrancar en calent des de
\texttt{best.zip}~F6. La durada prevista és de 20--30M passos addicionals, suficient
per cobrir el rang on els enfonsaments reals del Nash gap són més freqüents.

\emph{Protocol d'avaluació definitiu.} Amb l'avaluació precisa actual
(taula~\ref{tab:checkpoint3} i figura~\ref{fig:checkpoint3-candidats}), \emph{cap} dels
tres candidats compleix simultàniament \(\text{exploit\_vs\_sl}<8\)~pp i
\(\text{metric\_robust}\geq70\%\): per tant, mentre no es disposi del re-run, el model
operatiu per defecte és \texttt{best.zip}~F6, que té la \texttt{metric\_robust} més alta
(69{,}2\%) i el Nash gap més proper al llindar. Quan el re-run estigui disponible, el seu
\texttt{best\_nash} s'avaluarà amb el mateix protocol i s'adoptarà per al desplegament si
baixa del llindar de 8~pp mantenint una robustesa equiparable.
