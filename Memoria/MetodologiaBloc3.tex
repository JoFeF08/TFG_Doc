...
zona Nash (<8~pp) en 25~ocasions a partir del pas~5M, confirman que el sistema arriba
a l'equilibri de forma recurrent quan la política \ac{SL} ja disposa de prou
historial. No obstant, el \emph{checkpoint} \texttt{best\_nash.zip} va ser seleccionat
al pas~1M, quan el Reservoir Buffer és gairebé buit: \(\text{exploit\_vs\_sl}=3{,}3\)~pp
en aquell moment reflecteix la incapacitat del \ac{SL} incipient de competir contra
la millor resposta i no un equilibri de Nash genuí. La hipòtesi resta pendent de
validació definitiva fins a la re-execució amb el criteri corregit
...
\begin{itemize}
  ...
  \item \textbf{\texttt{best\_nash.zip}} (\ac{PPO} Policy Net, Nash gap mínim): el
        ...
        \(\text{exploit\_vs\_sl}=3{,}3\)~pp. \emph{Nota}: aquest valor és
        probablement un artefacte del criteri de selecció sense llindar mínim de
        passos (Reservoir Buffer gairebé buit al pas~1M); la limitació es
        documenta a la secció~\ref{sec:checkpoint3}.
...
(taula~\ref{tab:checkpoint3}). Les mètriques de \texttt{best.zip} provenen del log
d'entrenament al \emph{checkpoint} de màxima \texttt{metric}; les dels altres dos pesos
requereixen una avaluació independent sobre el pool complet, que s'ha de fer quan el
re-run estigui disponible.

\begin{table}[H]
  \centering
  \begin{tabular}{lcccc}
    \toprule
    Model & \texttt{metric} (pic) & \texttt{metric\_robust} & \texttt{exploit\_vs\_sl} & \texttt{std\_pool} \\
    \midrule
    \texttt{best.zip}~F6       & 91{,}0\% @14M & 75{,}2\% & 16{,}7~pp    & 13{,}4\% \\
    \texttt{best\_nash.zip}~F6 & —             & —         & 3{,}3~pp @1M\textsuperscript{†} & — \\
    \texttt{sl\_final.pt}~F6   & —             & —         & —             & — \\
    \bottomrule
  \end{tabular}
  \caption{Candidats a desplegament al tancament de la Fase~6. Les cel·les «—» indiquen
           que l'avaluació independent sobre el pool complet és pendent del re-run
           planificat. \textsuperscript{†}Valor probablement espuri (vegeu la
           limitació al text).}\label{tab:checkpoint3}
\end{table}
...
el mínim de l'entrenament. Al pas~1M, però, el Reservoir Buffer acumula menys de 20k
mostres (màxim: 200k): la política \ac{SL} no té prou historial per representar la
distribució d'accions de l'\ac{AR} i \texttt{exploit\_vs\_sl} resulta artificialment
baix. A més, amb 30~partides d'avaluació la granularitat és de 3{,}3~pp per partida,
de manera que el valor mínim observable coincideix exactament amb el valor mesurat.

La figura~\ref{fig:fase6-nash} mostra, en canvi, que a partir del pas~5M la corba
entra 25~vegades per sota del llindar de 8~pp al llarg de tot l'entrenament
(10--20M: 4~cops; 20--30M: 3; 30--40M: 4; 40--50M: 3). Això confirma que el
sistema assoleix la zona Nash de forma recurrent quan el \ac{SL} disposa de prou
historial.
...
(1)~un llindar mínim de \(10^7\) passos que impedeixi la selecció abans del pas~10M;
(2)~augmentar les partides d'avaluació de 30 a 60 (resolució $\approx$1{,}7~pp); i
(3)~arrancar en calent des de \texttt{best.zip}~F6. La durada prevista és de 20--30M
passos addicionals, suficient per cobrir el rang on els enfonsaments reals del Nash
gap són més freqüents.

\emph{Protocol d'avaluació definitiu.} Quan el re-run estigui disponible, es
compararà cada candidat sobre les quatre mètriques de la taula~\ref{tab:checkpoint3}
i sobre el win-rate per variant del pool. S'adoptarà el model que assoleixi
\(\text{exploit\_vs\_sl}<8\)~pp (amb el nou criteri) i
\(\text{metric\_robust}\geq70\%\). Si cap candidat compleix el llindar de Nash,
\texttt{best.zip}~F6 s'adoptarà com a model de desplegament per defecte.
