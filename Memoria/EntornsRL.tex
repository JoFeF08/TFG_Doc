...
\chapter{Entorns d'Aprenentatge per Reforç}
\label{cap:entorns}
...
\begin{enumerate}
  \item \textbf{Capa RLCard} (\texttt{TrucEnv} / \texttt{TrucEnvMa}): hereta
        de la classe base \texttt{Env} de RLCard i encapsula el bucle de joc.
        S'encarrega de convertir l'estat intern del motor en observacions estructurades
        (mètode \texttt{\_extract\_state}), d'exposar les accions legals en cada pas
        (\texttt{\_get\_legal\_actions}) i de recollir les trajectòries completes.

  \item \textbf{Capa Gymnasium} (\texttt{TrucGymEnv} / \texttt{TrucGymEnvMa}):
        embolcalla la capa RLCard amb la interfície estàndard de Gymnasium (mètodes
        \texttt{reset} i \texttt{step}) per fer-la compatible amb \acf{SB3}.
        Gestiona els torns de l'oponent (\texttt{\_skip\_opponent\_turns}) i acumula
        els rewards intermedis que es generen entre dues accions consecutives de
        l'agent aprenent.
\end{enumerate}
...
\begin{itemize}
  \item \texttt{TrucGymEnvSessio} (\texttt{joc/entorn\_ma/gym\_env\_sessio.py}):
        l'episodi agrupa $N$ mans consecutives contra el mateix oponent (per defecte
        $N=5$). L'oponent se samplea del \emph{pool} a cada \texttt{reset()}, cosa
        que obliga l'agent a acumular context entre mans i potencialment detectar els
        patrons de l'oponent. S'ha usat a la Fase~4 (memòria cross-mans, \ac{DRQN}).

  \item \texttt{parallel\_env\_ma.py}: procés fill (\texttt{worker}) que executa
        \texttt{TrucEnvMa} en un procés separat via \texttt{multiprocessing} i
        retorna les trajectòries amb els rewards intermedis al procés principal.
        Permet col·lectar dades de múltiples episodis en paral·lel durant l'entrenament
        propi de \ac{NFSP}.
\end{itemize}

El mètode \texttt{set\_opponent()} de \texttt{TrucGymEnv} permet canviar l'agent
oponent en calent sense reconstruir l'entorn; això és imprescindible per als callbacks
d'entrenament que implementen el pool d'oponents i el curriculum.

\section{Espai d'observació}
\label{sec:observacio}

L'observació que rep l'agent en cada pas és un diccionari amb dues entrades
complementàries, generades per \texttt{\_extract\_state} a \texttt{env.py}:
...
L'espai d'accions conté 19 accions discreta, definides al capítol~\ref{cap:joc}. No
...
El mètode \texttt{\_get\_legal\_actions()} retorna un \texttt{OrderedDict} amb les
accions vàlides en el pas actual. En els wrappers Gymnasium, aquest conjunt
s'emmagatzema a \texttt{self.\_legal\_actions} i s'usa de dues maneres:

\begin{itemize}
  \item \textbf{Fallback d'acció il·legal}: si el model prediu una acció fora del
        conjunt legal, \texttt{TrucGymEnv.step()} la substitueix per la primera
        acció legal (\texttt{legal\_actions[0]}). Això garanteix que l'entorn mai
        no genera errors per accions invàlides durant les primeres fases d'entrenament,
        quan el model és quasi aleatori.
  \item \textbf{Màscara explícita}: en algoritmes que ho suporten (com el \ac{PPO}
        amb màscares de \ac{SB3}), es pot passar la llista de booleà directament
        al model per forçar que la distribució de política sigui zero en accions
        il·legals.
\end{itemize}
...
\texttt{get\_payoffs()} retorna la recompensa final quan la partida (o la mà, en el
cas de \texttt{TrucEnvMa}) acaba:
...
El motor emmagatzema les recompenses intermèdies al camp
\texttt{reward\_intermedis} de cada transició. Atès que RLCard posa per
disseny recompensa zero a totes les transicions intermèdies, la funció
\texttt{reorganize\_amb\_rewards} postprocessa les trajectòries un cop
acabat l'episodi: per a cada transició extreu el camp \texttt{reward\_intermedis}
del \texttt{raw\_obs} i substitueix el zero pel valor acumulat de l'equip,
deixant intacte el \emph{payoff} final.

Als wrappers Gymnasium, \texttt{\_reward\_from\_raw()} extreu directament el reward
intermedi en cada crida a \texttt{step()} i l'amplifica per un factor~5:
...
La variant per mans (\texttt{TrucGameMa}) simplifica el sistema: acumula un únic
reward net al final de cada mà com $\pm\,\text{punts}/24$, sense senyals de ronda
individuals. Això afavoreix un aprenentatge més estable per a la fase de
\emph{curriculum learning} on cada episodi és una única mà.
