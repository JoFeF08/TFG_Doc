...
La Fase~3 estudia un \emph{feature extractor} convolucional propi, anomenat COS
(\texttt{CosMultiInput}), com a alternativa a l'extractor pla per defecte de \ac{SB3}.
L'estudi es divideix en dues preguntes acoblades, tractades en dues subsubseccions: si
l'\textbf{arquitectura} convolucional aporta valor per si sola (Q1,
secció~\ref{sec:fase3-arq}) i, en cas que la representació importi, si
\textbf{inicialitzar-la} amb coneixement previ del joc desbloqueja un rendiment superior
(Q2, secció~\ref{sec:fase3-init}).
...
l'observació) sense cap altre canvi. L'adaptador \texttt{CosMultiInputSB3}
(\texttt{RL/models/sb3/sb3\_features\_extractor.py}) fa de pont entre el vector pla i les
dues branques.
...
un agent sense estat intern. La Fase~4 introdueix dues variables: una memòria seqüencial
(\acf{LSTM}) que manté estat entre mans, i un \emph{pool} de sis variants d'oponent
(taula~\ref{tab:pool-variants}). La hipòtesi és que l'\ac{LSTM}, entrenada en sessions de
\(N=5\) mans consecutives contra el mateix oponent (entorn \texttt{TrucGymEnvSessio}),
aprendrà a detectar el patró de l'oponent i s'hi adaptarà, amb un win-rate creixent dins
la sessió.
...
\texttt{sb3\_contrib}) amb \emph{retropropagació en el temps truncada} sobre seqüències, on
el \texttt{batch\_size} es mesura en seqüències i no en transicions individuals. No s'usa
\acs{DRQN} (l'anàleg recurrent del DQN) perquè requeriria un \emph{replay buffer} seqüencial
no disponible a \ac{SB3}.
...
(taula~\ref{tab:fase4}):
\begin{itemize}
  \item \textbf{H1 falla}: l'\ac{LSTM} no millora el rendiment global. F4-complet (82{,}0\%)
        queda 4~punts \emph{per sota} de F4-ablació (86{,}0\%), amb un cost computacional
        46$\times$ superior (15{,}2~h vs 0{,}33~h).
  \item \textbf{H2 falla}: no hi ha adaptació clara dins la sessió. La corba de win-rate per
        posició és sorollosa i no monòtona
        (\([77{,}5;\,73{,}2;\,80{,}7;\,71{,}1;\,78{,}9]\), \(\Delta = +1{,}4\)~punts) i el
        mateix patró apareix a la versió sense memòria, cosa que indica un artefacte de la
        mida de mostra (\(\sim\)56~sessions) i no aprenentatge adaptatiu real.
  \item \textbf{H3 parcial}: F4-ablació queda 3{,}5~punts sota PPO frozen, just al límit del
        criteri (\(\pm3\)~punts); el pool dificulta lleugerament la convergència sense
        memòria.
\end{itemize}
La causa principal del fracàs de l'\ac{LSTM} és el \textbf{desajust entre entrenament i
avaluació}: s'entrena sobre sessions de 5~mans (\(\sim\)50~passos), però s'avalua sobre
sessions de 5~partides senceres (\(\sim\)500~passos), un context molt més llarg que la xarxa
no ha vist mai durant l'entrenament. S'hi sumen el pressupost reduït (12M vs 24M de les
línies base) i les sessions d'entrenament massa curtes perquè la retropropagació propagui
senyals d'adaptació.
...
\paragraph{Decisió.} L'\ac{LSTM} no justifica el seu cost amb el disseny actual, de manera
que la memòria queda descartada per al model final. La tria entre els candidats restants
(F4-ablació i les línies base de la Fase~3.5) es resol al Checkpoint~2.
...
contra un oponent fix i no s'ha provat contra el pool; per això no es tria. L'extensió natural
seria entrenar DQN amb el pool d'oponents per combinar el seu rendiment amb la robustesa,
però no s'ha executat per limitacions de temps. Finalment, l'\ac{LSTM} (F4-complet) queda
