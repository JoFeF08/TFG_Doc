...
\begin{table}[H]
  ...
  \begin{tabular}{llll}
    ...
    PPO-SB3     & \ac{SB3} & On-policy       & 48 entorns (\texttt{SubprocVecEnv}) \\
    \bottomrule
  \end{tabular}
...
\texttt{RandomAgent} (per mantenir diversitat i explorar estats poc habituals) i un gruix
d'\texttt{AgentRegles} (el senyal estratègic principal). La diferència és el
...
\begin{table}[H]
  ...
  \begin{tabular}{lccc}
    \toprule
    Agent & Random & \texttt{AgentRegles} & Self-play \\
    \midrule
  ...
  \caption{Distribució d'oponents per episodi a la Fase~1. Els agents de RLCard
           ...
           (\texttt{RandomAgent} i \texttt{AgentRegles}).}
...
no memorització, l'\texttt{AgentRegles} de l'avaluació s'instancia amb una llavor diferent
de la d'entrenament.
...
entrenar primer sobre mans (amb l'entorn \texttt{TrucGameMa}/\texttt{TrucEnvMa} descrit al
...
\begin{enumerate}
  \item \textbf{Etapa de mans} (12M passos): l'agent s'entrena a \texttt{TrucGameMa}, on
  ...
  \item \textbf{Etapa de partides} (12M passos): els pesos apresos es transfereixen a
        \texttt{TrucGame}, on l'episodi és una partida sencera fins a 24~punts. L'agent
